...
## Annexe R6 — Étage 2 embeddings : infrastructure livrée, verrouillée (2026-07-23)

R6 matérialise l'étage 2 annoncé en tête de ce document. Décisions tranchées et
datées du chantier (D1–D14, spec R6 §14 ; preuves, rondes et journal de build
dans `router/CONVERGENCE_LEDGER.md`) :

- **D1 — Composition** : l'étage 2 est un `TwoStageRouter(stage1, stage2)`
  derrière le `SafeRouter` ; nouvelle `router_version="embed_v0"` (canary org
  par org, patron ml_v05). L'interface `Router.decide` reste stable, cascade de
  construction FAIL-SOFT : étage 2 absent/KO ⇒ étage 1 (ml:v05) ; étage 1 KO ⇒
  `fallback:heuristic` — l'API répond 200 dans tous les cas (invariant §5.2).
- **D2 — Triple verrou opt-in, OFF par défaut partout** : l'étage 2 ne VOIT le
  texte que si (1) l'env serveur `SOBRIO_EMBED_STAGE2` vaut EXACTEMENT `"1"`
  (jamais posé en CI), (2) la politique d'org porte À LA FOIS
  `router_version == "embed_v0"` ET `send_prompt_text == True` (strict, défaut
  contractuel `false`), (3) la requête porte `prompt_text`. Tout verrou fermé ⇒
  texte DÉTRUIT dès `routes.py`, décision étage 1, réponse 200 normale —
  l'échec est silencieux pour le client (matrice 8/8 prouvée en E2E).
- **D3 — Arbitrage et plafond 0,74** : l'étage 2 ne s'exécute que si la
  confiance de l'étage 1 < 0,75 ; il ne l'emporte que si sa confiance est
  supérieure. Plafond **0,74** porté par `confidence_cap` du metadata (SEULE
  source, fail-closed — le retirer en v1 = nouvel artefact, pas un patch),
  appliqué UNIQUEMENT dans `EmbedHead.predict` : une seule chaîne de confiance,
  servie ET mesurée — la tête v0 ne peut JAMAIS déclencher l'auto-bascule sans
  clic (RFC-0003, seuil 0,75).
- **D4 — Statut de production (point TRANCHÉ)** : le corpus 30k n'a aucun
  texte — R6 livre l'INFRASTRUCTURE verrouillée (chargement, gardes, refus,
  repli, gate, rotation, bench), et la tête v0 est entraînée sur embeddings
  SYNTHÉTIQUES : « tête v0 fixtures synthétiques — non représentative, attend
  télémétrie v1 » (statut consigné mot pour mot dans le metadata de
  l'artefact). `heads/promoted/` reste VIDE en production : même triple verrou
  ouvert, le bridge sert l'étage 1 seul tant qu'aucune tête n'est promue ; la
  promotion de la tête v0 synthétique est un acte de staging/démo explicite.
  **La tête réelle attend la télémétrie v1** (`events_reco` + opt-in texte,
  chantier R7/V1).
- **D5 — Variante int8 par défaut** ; qualité int8 vs fp32 tranchée en v1 sur
  données réelles ; manifest bi-variante avec provenance d'export PAR variante,
  vérifiée par sha256, jamais présumée.
- **D6 — Fixtures** : vecteurs synthétiques seedés régénérés à la demande,
  hash canonique épinglé committé (< 5 Ko), JAMAIS de texte.
- **D7 — Verrou pickle** : `__reduce__` conditionnel sur `PromptSignals`
  (porteur de texte ⇒ TypeError, message sans contenu) + scan statique
  asdict/astuple/pickle/vars en glob sur tout le code de production.
- **D8 — Gate et promotion** : réutilisation d'`evaluate_gate` 100 % pur
  (`--suite embed --budget-ms 30`, bornes inclusives) ; la promotion exige EN
  PLUS une preuve bench RÉELLE fraîche (p95, RSS, sha encodeur du manifest) —
  fail-closed, exit 2 sinon.
- **D9 — Fetch** : CLI dédié `router/tools/fetch_embed_model.py`, flag
  `SOBRIO_ALLOW_MODEL_DOWNLOAD="1"` vérifié avant tout, sha256 du manifest
  obligatoires (écart ⇒ suppression + refus), garde AST — imports réseau
  UNIQUEMENT dans `_download()` ; schémas d'URL restreints à https/file
  (durci ronde 1).
- **D10 — Pins littéraux** : feature_spec v1 (étage 1, bit-intact en R6) et
  embed_spec pinnés INTÉGRALEMENT en tests stdlib toujours exécutés ; les
  littéraux sha d'`embed.py` sont verrouillés égaux au manifest int8 par test
  croisé (ronde 1).
- **D11/D12 — Pipeline e5 canonique** : troncature mémoire 4 000 caractères
  puis 256 tokens (valeur CANDIDATE non mesurée — voir recadrage ci-dessous),
  préfixe `query: `, mean pooling MASQUÉ, normalisation L2, dim 384, threads
  ORT = 1. AUCUN changement de contrat : le contrat v1.x portait déjà
  `prompt_text` optionnel et `send_prompt_text` (défaut `false`).
- **D13 — Baseline du gate embed** : tête « prior » constante sonnet@0.5 —
...
— le choix et l'approbation du dépôt exportateur appartiennent au fondateur.
Conséquences livrées : manifest committé à sources `null`, littéraux
`_ENCODER_SHA256`/`_TOKENIZER_SHA256` à `None` (tout encodeur local est refusé
fail-closed, `EmbedLoadError` propre), spike `max_tokens`, bench réel p95/RSS
et premier fetch vérifié DIFFÉRÉS au geste fondateur ; `promote_embed` refuse
donc toute promotion effective (exit 2 explicite) — comportement voulu.

**Budgets (§7) tenus par construction et par gardes** : étage 2 p95 ≤ 30 ms
CPU (borne INCLUSIVE, gate `--budget-ms 30` + garde bench D8), RSS < 1 Go
(strict, conversion `ru_maxrss` par plateforme), artefact < 500 Mo, rien
> 20 Mo commité, dépense API 0,00 $. La mesure RÉELLE du pipeline complet est
différée au geste fondateur — harnais, bench et gardes sont livrés et testés.
